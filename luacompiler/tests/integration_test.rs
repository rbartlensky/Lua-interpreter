...
fn ldi_generation() {
    ...
    let bc = pt.unwrap().compile_to_ir();
...
#[test]
fn ldf_generation() {
    let pt = LuaParseTree::from_str(String::from("x = 2.0"));
    let bc = pt.unwrap().compile_to_ir();
    assert_eq!(bc.instrs_len(), 2);
    assert_eq!(bc.reg_count(), 2);
...
#[test]
fn lds_generation() {
    let pt = LuaParseTree::from_str(String::from("x = \"1.2\""));
    let bc = pt.unwrap().compile_to_ir();
    assert_eq!(bc.instrs_len(), 2);
    assert_eq!(bc.reg_count(), 2);
...
fn assert_bytecode(opcode: Opcode, operation: &str) {
    let pt = LuaParseTree::from_str(String::from(format!("x = 1 {} 2", operation)));
    let bc = pt.unwrap().compile_to_ir();
